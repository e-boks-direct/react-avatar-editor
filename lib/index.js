...
var _loadImageUrl = require("./utils/load-image-url");

var _loadImageUrl2 = _interopRequireDefault(_loadImageUrl);

var _loadImageFile = require("./utils/load-image-file");
...
var AvatarEditor = (_temp2 = _class = function (_React$Component) {
  ...
  AvatarEditor.prototype.componentDidMount = function componentDidMount() {
  ...
  AvatarEditor.prototype.componentWillReceiveProps = function componentWillReceiveProps(newProps) {
  ...
  AvatarEditor.prototype.componentDidUpdate = function componentDidUpdate(prevProps, prevState) {
    // eslint-disable-next-line react/no-find-dom-node
  ...
  AvatarEditor.prototype.componentWillUnmount = function componentWillUnmount() {
    if (document) {
      var nativeEvents = deviceEvents.native;
  ...
  AvatarEditor.prototype.isVertical = function isVertical() {
    return this.props.rotate % 180 !== 0;
  };

  AvatarEditor.prototype.getBorders = function getBorders() {
    var border = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.props.border;

    return Array.isArray(border) ? border : [border, border];
  };

  AvatarEditor.prototype.getDimensions = function getDimensions() {
    ...
    var _getBorders = this.getBorders(border),
        borderX = _getBorders[0],
        borderY = _getBorders[1];
  ...
  AvatarEditor.prototype.getImage = function getImage() {
  ...
  AvatarEditor.prototype.getImageScaledToCanvas = function getImageScaledToCanvas() {
  ...
  AvatarEditor.prototype.getXScale = function getXScale() {
  ...
  AvatarEditor.prototype.getYScale = function getYScale() {
  ...
  AvatarEditor.prototype.getCroppingRect = function getCroppingRect() {
  ...
  AvatarEditor.prototype.loadImage = function loadImage(image) {
    if (isFileAPISupported && image instanceof File) {
      this.loadingImage = makeCancelable((0, _loadImageFile2.default)(image)).promise.then(this.handleImageReady).catch(this.props.onLoadFailure);
    } else if (typeof image === "string") {
      this.loadingImage = makeCancelable((0, _loadImageUrl2.default)(image, this.props.crossOrigin)).promise.then(this.handleImageReady).catch(this.props.onLoadFailure);
    }
  };

  AvatarEditor.prototype.getInitialSize = function getInitialSize(width, height) {
  ...
  AvatarEditor.prototype.paintImage = function paintImage(context, image, border) {
  ...
  AvatarEditor.prototype.calculatePosition = function calculatePosition(image, border) {
    image = image || this.state.image;

    var _getBorders2 = this.getBorders(border),
  ...
  AvatarEditor.prototype.paint = function paint(context) {
    context.save();
    ...
    var _getBorders3 = this.getBorders(dimensions.border),
        borderSizeX = _getBorders3[0],
        borderSizeY = _getBorders3[1];
  ...
  AvatarEditor.prototype.render = function render() {
    ...
    return _react2.default.createElement("canvas", _extends({ ref: this.setCanvas }, attributes, rest));
  };
...
AvatarEditor.propTypes = process.env.NODE_ENV !== "production" ? {
  scale: _propTypes2.default.number,
  rotate: _propTypes2.default.number,
  image: _propTypes2.default.oneOfType([_propTypes2.default.string, _propTypes2.default.instanceOf(Blob)].concat(isFileAPISupported ? [_propTypes2.default.instanceOf(File)] : [])),
  border: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.arrayOf(_propTypes2.default.number)]),
  ...
  disableBoundaryChecks: _propTypes2.default.bool,
  disableHiDPIScaling: _propTypes2.default.bool
} : {};
